...
TcpServer::TcpServer(const std::string & ip, const uint32_t port, int thread_num, int timeout)
    // 指定时间循环是否为主事件循环
    : m_evloop(new EventLoop(new Epoll(timeout), true)), m_thread_num(thread_num), m_acceptor(m_evloop.get(), ip, port), m_tp(new ThreadPool(m_thread_num, "IO"))
{
    ...
    {
        // 创建临时指针变量，然后移动给容器中的 指针变量，这里创建的都是从事件循环
        std::unique_ptr<EventLoop> temp_up(new EventLoop(new Epoll(timeout), false));
...
void TcpServer::close_connection(spConnection con)
{
    ...
    m_subevloops[con->get_fd() % m_subevloops.size()]->remove_Connection(con);
    // 加锁是因为 TcpServer 有主线程和子线程共同在操作，因此这里需要加锁操作自己的 Connection map
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
// 注意，因为 子事件循环中才会调用该函数，因此，该函数会运行在 子线程中，这和前面的 map 相关操作不在同一个线程中，因此这里需要加锁
