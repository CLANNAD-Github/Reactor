...
{
    m_tsp = Timestamp::now(); // 更新时间戳，避免被误删
    ...
    {
        // printf("send() 在IO事件循环中 threadid:%d\n", syscall(SYS_gettid));
    ...
    while (true)
    {
        ...
        if (nread == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) // 全部的数据已经读取完毕
        {
            while (true)
            {
                ...
                // printf("Connection pickmessage:%s\n", message->c_str());
...
std::string Connection::get_ip() const { return m_client_socket->get_ip(); }
uint16_t Connection::get_port() const { return m_client_socket->get_port(); }
int Connection::get_fd() const { return m_client_socket->get_fd(); }
